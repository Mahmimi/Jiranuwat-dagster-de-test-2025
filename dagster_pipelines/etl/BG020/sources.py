...
@dlt.resource(
    table_name="bg020_user_data",
    write_disposition="replace",
    name="BG020_user_xlsx",
)
def read_bg020_user_excel() -> Iterator[Dict[str, Any]]:
    """
    Stream rows from the sheets of a BG020 Excel file from User.
    Dynamically mark all empty-value columns as text using dlt.mark.with_hints.
    """
    time.sleep(5)

    local_dir = Path("dagster_pipelines/data")
    file_path = local_dir / Path(load_downloaded_filename(file_key="user"))
...
@dlt.resource(
    table_name="bg020_sap_data",
    write_disposition="replace",
    name="BG020_sap_xlsx",
)
def read_bg020_sap_excel() -> Iterator[Dict[str, Any]]:
    """
    Stream rows from the sheets of a BG020 Excel file from SAP.
    Dynamically mark all empty-value columns as text using dlt.mark.with_hints.
    """
    time.sleep(5)

    local_dir = Path("dagster_pipelines/data")
    file_path = local_dir / Path(load_downloaded_filename(file_key="sap"))
    size = os.path.getsize(file_path)
...
def bg020_source() -> List[dlt.resource]:
    """
    Create a dlt source that reads data from the BG020 Excel files from both User and SAP.
    Args:
    ...
    return [read_bg020_user_excel, 
            read_bg020_sap_excel, ]  # Return the resource for reading the Excel file
